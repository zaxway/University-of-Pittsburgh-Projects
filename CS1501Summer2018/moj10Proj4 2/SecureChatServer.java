...
public class SecureChatServer {
    ...
    public SecureChatServer(int MaxU) throws IOException
    {
		...
		E = new BigInteger(StringE); // exponent
		D = new BigInteger(StringD); // decryption ed === 1(mod((p-1)(q-1)))
		N = new BigInteger(StringN); // N = pq --> mod(N)
		R = new Random();
		...
		while (true)
		{
			if (numUsers < MaxUsers)
			{
				try
				{
					...
					if (test > 0.5)
						encType = new String("Sub"); // change this back to "Sub"
					else
